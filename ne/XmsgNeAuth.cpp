...
void XmsgNeAuth::handle(shared_ptr<XscChannel> channel, SptrXitp trans, shared_ptr<XmsgNeAuthReq> req)
{
	...
	shared_ptr<XmsgImAuthCfgXmsgNeN2hAddr> cfgNe = XmsgNeAuth::findXmsgNeCfg(req->cgt());
	if (cfgNe == nullptr)
	{
	...
	rsp->set_cgt(XmsgImAuthCfg::instance()->cfgPb->cgt());
	trans->end(rsp);
	if (old == nullptr)
		return;
...
shared_ptr<XmsgImAuthCfgXmsgNeN2hAddr> XmsgNeAuth::findXmsgNeCfg(const string& cgt)
{
	for (int i = 0; i < XmsgImAuthCfg::instance()->cfgPb->n2h_size(); ++i) 
	{
		auto ne = XmsgImAuthCfg::instance()->cfgPb->n2h(i);
		if (ne.cgt() != cgt)
			continue;
		shared_ptr<XmsgImAuthCfgXmsgNeN2hAddr> cfgNe(new XmsgImAuthCfgXmsgNeN2hAddr());
		cfgNe->CopyFrom(ne);
		return cfgNe;
	}
	return nullptr;
